...
import { Book, ReviewInterface } from '../Pages/interface';
...
export const useReviewList = () => {
  ...
  const getReviewList = (setIsLoading: (arg: boolean) => void) => {
    setIsLoading(true);
    axios
      .get<Book[]>(`${apiUrl}/books?offset=0`, {
        headers: {
...
export const useNewReview = () => {
  ...
  const postNewReview = ({ title, url, detail, review }: ReviewInterface) => {
    axios
      .post(
        `${apiUrl}/books`,
        {
...
export const useDeleteReview = () => {
  ...
  const deleteReview = ({ id }: { id: string | undefined }) => {
    axios
